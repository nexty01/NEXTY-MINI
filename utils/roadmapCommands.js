...
async function executeOwner(kind, ctx) {
    ...
    if (kind === 'healthcheck' || kind === 'diagnostics') {
        ...
        const keys = ['AGNES_API_KEY', 'GROQ_API_KEY', 'OPENAI_API_KEY', 'OPENROUTER_API_KEY', 'GEMINI_API_KEY'].filter(k => process.env[k]).join(', ') || 'none';
...
async function fetchText(url) {
    const response = await axios.get(url, { timeout: 15000, maxContentLength: 2 * 1024 * 1024, headers: { 'User-Agent': 'NEXTY-MINI/3.0' } });
...
async function executeMedia(kind, ctx) {
    ...
    const input = path.join(os.tmpdir(), `nexty-${Date.now()}-${mediaType}`);
