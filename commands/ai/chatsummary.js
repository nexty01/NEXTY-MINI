...
const memory = require('../../utils/nextyMemory');
const nexty = require('./nexty');
...
module.exports = {
    ...
    async execute({ from, sender, isGroup, args, reply, database }) {
        ...
        if (!memory.isEnabled(database, chatKey)) return reply('🧠 Memory is OFF for this chat. Use .nexty memory on first.');
        ...
        const answer = await nexty.getNextyAIReply(prompt, `summary:${chatKey}`, { memoryContext: context });
