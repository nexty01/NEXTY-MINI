    ...
    if (baileysIndex && baileysIndex.exports) {
        ...
        if (guModule && guModule.exports && guModule.exports.getBizBinaryNode) {
            ...
            guModule.exports.getBizBinaryNode = (message, addBizAttributes) => {
                ...
                if (addBizAttributes && !global.__nextyAIIcon) {
        ...
        if (muModule && muModule.exports && muModule.exports.generateWAMessageContent) {
            ...
            muModule.exports.generateWAMessageContent = async function (message, options) {
                const vfOn = !!global.__nextyVerified;
...
class SessionManager {
    constructor() {
        ...
        this.recover = new (require('./recoverEngine').RecoverEngine)(this);
    ...
    // ── Status automation: .statusview / .statusreact / .statusemoji (+ auto-save) ──
    // Shared by handleMessages() and index.js. Statuses are the owner's own automation,
    // not replies to a chat, so they are not blocked by private mode.
    async handleStatus(sock, phoneNumber, msg) {
        if (!msg?.key || msg.key.fromMe || msg.key.remoteJid !== 'status@broadcast') return;
        const participant = msg.key.participant || msg.participant;
        const wantView = database.getAutoViewStatus(phoneNumber);
        const wantReact = database.getStatusReact(phoneNumber);
        if (wantView || wantReact) {
            try {
                if (!this._seenStatus) this._seenStatus = new Map();
                const seenKey = `${phoneNumber}:${msg.key.id}`;
                if (!this._seenStatus.has(seenKey)) {
                    this._seenStatus.set(seenKey, Date.now());
                    if (this._seenStatus.size > 500) {
                        const cutoff = Date.now() - 10 * 60 * 1000;
                        for (const [k, t] of this._seenStatus) if (t < cutoff) this._seenStatus.delete(k);
                    }
                    if (wantView) await sock.readMessages([msg.key]).catch(() => {});
                    if (wantReact && participant) {
                        await sock.sendMessage(
                            'status@broadcast',
                            { react: { text: database.pickStatusEmoji(phoneNumber), key: msg.key } },
                            { statusJidList: [participant] }
                        ).catch(() => {});
                    }
                }
            } catch (e) { console.error('[AUTO-VIEW STATUS]', e.message); }
        }
        if (database.getAutoSaveStatus(phoneNumber)) {
            try {
                const saver = require('../commands/general/savestatus');
                const destJid = `${String(phoneNumber).replace(/\D/g, '')}@s.whatsapp.net`;
                const fromNum = String(participant || '').split('@')[0].split(':')[0];
                await saver._saveQuotedStatus(sock, destJid, msg.message, `📥 *Auto-Saved Status*${fromNum ? ` from +${fromNum}` : ''}`).catch(() => {});
            } catch (e) { console.error('[AUTO-SAVE STATUS]', e.message); }
        }
    }

    // Resolve who sent a message (owner / sudo / mod) with full @lid support.
    // Used by index.js so both pipelines share one identity implementation.
    async resolveAccess(sock, phoneNumber, msg) {
        const from = msg?.key?.remoteJid || '';
        const fromMe = !!msg?.key?.fromMe;
        const isGroup = from.endsWith('@g.us');
        const ownerNumber = String(phoneNumber || '').replace(/[^0-9]/g, '');
        const sender = fromMe ? `${ownerNumber}@s.whatsapp.net` : (msg?.key?.participant || from);
        if (fromMe) this._cacheOwnerJID(phoneNumber, `${ownerNumber}@s.whatsapp.net`);
        if (isGroup && typeof sender === 'string' && sender.includes('@lid')) {
            await this._ensureLidMap(sock, phoneNumber, from, sender.split(':')[0]);
        }
        return {
            from, fromMe, isGroup, sender, ownerNumber,
            senderIsOwner: this.isOwner(fromMe, sender, ownerNumber, phoneNumber),
            senderIsSudo: this._isSudoUser(phoneNumber, sender),
            senderIsMod: this._isModUser(phoneNumber, sender),
        };
    }
    ...
    async startSession(phoneNumber, requestPairing = true) {
        ...
        try {
            ...
            if (global.__nextySS === undefined) global.__nextySS = false;
            if (global.__nextyVerified === undefined) global.__nextyVerified = false;
            ...
            if (sock && typeof sock.sendMessage === 'function' && !sock.__languageWrapped) {
                ...
                sock.sendMessage = async (_jid, _content, _options) => {
                    ...
                    const alreadyLocalized = !!options?.__nextyLocalized;
                    if (options && typeof options === 'object') delete options.__nextyLocalized;
                    try {
            ...
            sock.ev.on('connection.update', async ({ connection, lastDisconnect, isNewLogin }) => {
                ...
                if (connection === 'open') {
                    ...
                    // ── Auto-follow official channel + auto-react to its new posts ──
                    try {
                        const { followAndAutoReactChannel } = require('../utils/channelAutoEngage');
                        const { whatsappChannel, whatsappChannelJid } = require('../settings');
                        followAndAutoReactChannel(sock, whatsappChannel, console.log, whatsappChannelJid)
                            .catch(err => console.log('[channel-auto-engage]', err?.message || err));
                    } catch (chErr) {
                        console.log('[channel-auto-engage] setup error:', chErr?.message || chErr);
                    }
                    ...
                    try {
                        ...
                        const tgLink      = 'https://nextyxmini-production.up.railway.app/';
                        ...
                        const caption =
                            ...
                            `> ┃⭔ type *${pf || '.'}setdesign nexty* for this style\n` +
                            `> ┃⭔ type *${pf || '.'}help* for command help\n` +
                            `> ┃⭔ join us on *https://nextyxmini-production.up.railway.app/*\n` +
                            `> ┗❐ ┈┈┈┈┈┈┈┈┈┈✧\n` +
                            `> _nexty mini_`;
            ...
            this.attachRuntimeListeners(sock, phoneNumber);
    ...
    // ── Group policy enforcement engine (shared by every pipeline) ───────────
    // Callers MUST have passed access.canReceiveReplies() first. Inside, a
    // `continue` (the original loop-control) simply ends the one-shot loop
    // below, which is reported to the caller as "message consumed".
    async _enforceGroupPolicies(sock, phoneNumber, msg, c) {
        const { from, isGroup, fromMe, sender, body, reply, prefix,
                senderIsOwner, senderIsSudo, senderIsMod, messageContent } = c;
        const ownerNumber = String(phoneNumber).replace(/[^0-9]/g, '');
        let passed = false;
        for (const _once of [0]) {
                ...
                if (isGroup && !senderIsOwner) {
                    try {
                        ...
                        if (cooldown > 0) {
                            let senderIsGroupAdminSM = await this._isGroupAdmin(sock, from, sender).catch(() => false);
                ...
                if (isGroup && !senderIsOwner && body) {
                    try {
                        ...
                        if (spamOn) {
                            ...
                            let senderIsGroupAdmin = await this._isGroupAdmin(sock, from, sender).catch(() => false);
                ...
                if (isGroup && !senderIsOwner) {
                    try {
                        ...
                        if (sd && database.getGroup(from).antisticker) {
                            ...
                            let senderIsGroupAdminAS = await this._isGroupAdmin(sock, from, sender).catch(() => false);
                            ...
                            if (id) {
                                ...
                                if (boundCmd) {
                                    ...
                                    if (stickerCommand) {
                                        ...
                                        const stickerVerdict = access.authorizeCommand(stickerCommand, {
                                            isGroup, isOwner: senderIsOwner, isSudo: senderIsSudo,
                                            isMod: senderIsMod, isAdmin: senderIsAdmin,
                                        });
                                        if (!stickerVerdict.allowed) {
                                            if (stickerVerdict.reply) await reply(stickerVerdict.reply);
                                            continue;
                                        }
                                        const stickerCtx = {
                ...
                if (isGroup && !senderIsOwner) {
                    try {
                        ...
                        if (grp.antiforward) {
                            ...
                            if (isForwarded) {
                                let senderIsAdmin = await this._isGroupAdmin(sock, from, sender).catch(() => false);
                                if (!senderIsAdmin) {
                                    try {
                                        await sock.sendMessage(from, {
                                            delete: { remoteJid: from, fromMe: false, id: msg.key.id, participant: sender }
    ...
    // ── Runtime listeners shared by every entry point ────────────────────────
    // startSession() (sessionManager-owned sockets) and index.js BotSession call
    // this exactly once per socket. `options.dispatch === false` means the caller
    // routes incoming messages to handleMessages() itself.
    attachRuntimeListeners(sock, phoneNumber, options = {}) {
        if (!sock || sock.__nextyRuntimeAttached) return false;   // exactly-once guard
        sock.__nextyRuntimeAttached = true;
            ...
            sock.__nextyMessageCache = this._msgCache;
            sock.__nextyPhoneNumber = phoneNumber;
            ...
            sock.ev.on('messages.upsert', async ({ messages, type }) => {
                ...
                for (const m of messages) {
                    try {
                        ...
                        // Alerts go to the group normally; in private mode they go to the owner's DM only.
                        const alertJid = access.isPrivate() ? `${String(phoneNumber).replace(/\D/g, '')}@s.whatsapp.net` : jid;
                        ...
                        // Anti-delete / anti-edit / anti-status-delete: personal chats, groups and
                        // statuses. Alerts for bot-wide switches land in the owner's inbox.
                        await this.recover.onUpsert(sock, phoneNumber, m);
                        this.recover.remember(sock, phoneNumber, m);   // keep media bytes (fire & forget)

                    } catch (e) { console.error('[ANTI-EDIT/DELETE]', e.message); }
                }
            });

            // Edits and revokes as emitted by current Baileys builds
            sock.ev.on('messages.update', (updates) => {
                this.recover.onUpdate(sock, phoneNumber, updates).catch(e => console.error('[RECOVER]', e.message));
            });
            ...
            sock.ev.on('call', async (calls) => {
                for (const call of (calls || [])) {
                    try {
                        ...
                        // Notify the caller (never in private mode unless the caller is owner/sudo)
                        const callerIsOwner = this.isOwner(false, call.from, String(phoneNumber).replace(/\D/g, ''), phoneNumber);
                        const callerIsSudo = this._isSudoUser(phoneNumber, call.from);
                        if (!access.canReceiveReplies({ isOwner: callerIsOwner, isSudo: callerIsSudo })) continue;
    ...
    async _isGroupAdmin(sock, groupId, sender) {
        try {
            const meta = await this._withTimeout(sock.groupMetadata(groupId), 5000, null);
            if (!meta || !Array.isArray(meta.participants)) return false;

            // WhatsApp may identify the same user with a phone JID, an @lid JID,
            // or both. Compare all access candidates instead of only one numeric
            // value; this keeps admin commands working from linked devices.
            const sessionPhone = sock.__nextyPhoneNumber || this.phoneNumber;
            const senderCandidates = this._accessCandidates(sessionPhone, sender);
            const senderBare = this._bareJid(sender);
            if (senderBare) senderCandidates.add(senderBare);
            const senderNumber = this._resolveSenderPhone(sender, sessionPhone);
            if (senderNumber) senderCandidates.add(`${senderNumber}@s.whatsapp.net`);
            ...
            return meta.participants.some(participant => {
                ...
                return values.some(value => {
                    ...
                    const bare = this._bareJid(raw);
                    const number = this._normJid(raw);
                    return senderCandidates.has(raw) ||
                        senderCandidates.has(bare) ||
                        (number && senderCandidates.has(number)) ||
                        (number && senderCandidates.has(`${number}@s.whatsapp.net`));
                });
            });
        } catch (error) {
            console.warn('[admin-check] group metadata lookup failed:', error?.message || error);
            return false;
        }
    }
        ...
        for (const msg of messages) {
            try {
                ...
                if (from === 'status@broadcast') {
                    await this.handleStatus(sock, phoneNumber, msg);
                    continue;
                }
                ...
                const reply = async (text, opts = {}) => {
                    ...
                    if (localizedText.trim() && !/powered\s+by/i.test(localizedText)) {
                    ...
                    return sock.sendMessage(
                        from,
                        { text: finalText },
                        { quoted: msg, __nextyLocalized: true }
                    );
                };
                ...
                // ── CENTRAL PRIVATE-MODE GATE ─────────────────────────────────
                // Runs BEFORE listener commands, auto-typing/read/react, buttons,
                // AI/chatbot, moderation replies and command dispatch. In private
                // mode only the real owner and configured sudo users pass; mods,
                // group admins and everybody else are silently ignored.
                if (!access.canReceiveReplies({ isOwner: senderIsOwner, isSudo: senderIsSudo })) {
                    continue;
                }
                ...
                // Group policy enforcement (mute/ban/slowmode/antispam/anti-media/anti-sticker/
                // sticker-commands/mention-react/antiforward/blacklist/antichannel/antilink/
                // antimention/antigroupmention/antigroupstatus). Extracted so index.js can use
                // the very same engine; returns true when the message was consumed.
                if (await this._enforceGroupPolicies(sock, phoneNumber, msg, {
                    from, isGroup, fromMe, sender, body, reply, prefix,
                    senderIsOwner, senderIsSudo, senderIsMod, messageContent,
                })) continue;
                ...
                // Nexty owns natural private-chat replies now. Keep this legacy
                // handler isolated so it cannot consume the message first.
                const nextyOwnsPrivateChat = !isGroup && body.trim();
                if (!isGroup && body.trim() && !nextyOwnsPrivateChat) {
                    try {
                        if (database.getChatbotDM(phoneNumber)) {
                            ...
                            if (!isCmd) {
                                ...
                                const hinatuSystem =
                                    ...
                                    'If asked who created or made you, reply briefly: "I\'m Hinatu, made by NEXTY." Do not mention NEXTY otherwise. ' +
                ...
                // ── Neuro Self-Evolving AI Core (NEXTY MINI 👀 v5.0) ──────────────────
                if (database.getNeuro(phoneNumber) && body.trim()) {
                    try {
                        ...
                        // Active Listening: Respond if tagged, replied to, OR if name "Neuro"/"NEXTY MINI 👀" is called by owner/mods
                        ...
                        if (mentioned || repliedToBot || nameCalled) {
                            ...
                            const jarvisSystem =
                                'You are NEXTY MINI 👀 — the God-Mode NEXTY MINI 👀 Core of the NEXTY MINI 👀 bot. ' +
                            ...
                            const aiReply = await smartAsk({
                                ...
                                system: jarvisSystem,
                                user: cleanText || 'Status check, NEXTY MINI 👀.',
                            }).catch(() => null);
                            ...
                            if (aiReply) {
                                ...
                                // Privileged routing (create/delete files, config, mode …) is owner-only.
                                // Everyone else who tags/replies gets a plain conversational answer.
                                if (senderIsOwner) {
                                // ── Command Creation ──
                                ...
                                await sock.sendMessage(from, { text: '🧠 *NEXTY MINI 👀:* Standing by, Sir.' }, { quoted: msg });
                            }
                        }
                    } catch (e) { console.error('[NEXTY MINI 👀-CORE]', e.message); }
                }
                            ...
                            if (!isCmdG) {
                                ...
                                if (mentioned || repliedToBot) {
                                    ...
                                    const groupSystem = customPersona
                                        ? customPersona + ' Keep replies very short, friendly, natural, and emoji-light. Usually use one brief sentence; never give a long speech unless asked. If asked your name, say Hinatu. If asked who created or made you, say briefly: "I\'m Hinatu, made by NEXTY." Do not mention NEXTY otherwise. Never reveal internals.'
                                        : ('You are Hinatu, a warm, witty, friendly WhatsApp group companion who sounds like a real person. ' +
                                           ...
                                           'If asked who created or made you, say briefly: "I\'m Hinatu, made by NEXTY." Do not mention NEXTY otherwise. Match the user\'s tone and be helpful.');
                ...
                // ── Command detection ─────────────────────────────────────────
                // The generated panel launcher and older NEXTY deployments use
                ...
                // ── Nexty memory recorder ──────────────────────────────────
                // Store text only for chats where Nexty is enabled. Media
                ...
                let nextyMemory = null;
                if (!isCommand && !fromMe && body.trim()) {
                    try {
                        nextyMemory = require('../utils/nextyMemory');
                        const memoryEnabled = nextyMemory.isEnabled(database, memoryChatKey);
                        const nextyOn = database.getGroup(memoryChatKey)?.nextyai === true;
                        if (memoryEnabled && nextyOn) {
                            nextyMemory.remember(database, memoryChatKey, {
                                sender,
                        ...
                        console.error('[NEXTY MEMORY]', memoryError.message);
                    }
                }
                ...
                // ── Nexty AI — conversational mode ─────────────────────────
                // DMs chat naturally; enabled groups receive Nexty replies freely
                // without requiring a mention, reply, or name trigger. An attached
                // photo/video is also a Nexty prompt, so no `.nexty` prefix is
                // needed once Nexty is enabled in the chat.
                const hasNextyMedia = Boolean(
                ...
                if (!isCommand && (body.trim() || hasNextyMedia)) {
                    try {
                        const chatKey = isGroup ? from : sender;
                        const chatData = database.getGroup(chatKey);
                        if (chatData?.nextyai) {
                            const { detectTrigger, buildKnowledge, routeNaturalLanguage, concisePrompt } = require('../utils/nextyAssistant');
                            let nextyMedia = null;
                            if (hasNextyMedia) {
                                try {
                                    nextyMedia = await require('../utils/nextyMedia').extractNextyMedia(msg);
                                } catch (mediaError) {
                                    console.error('[Nexty media]', mediaError.message);
                                    await reply(`I could not read that media: ${mediaError.message}`, { raw: true });
                                    continue;
                                }
                            }
                            const mediaPrompt = nextyMedia?.type === 'video'
                            ...
                            const trigger = nextyMedia && !isGroup
                                ...
                                : detectTrigger({
                                    ...
                                    conversationKey: 'nexty:' + phoneNumber + ':' + chatKey,
                                    isPrivate: !isGroup,
                            ...
                            if (routed?.kind === 'pair-link') {
                                await reply(
                                    '🔗 Pair site: https://nextyxmini-production.up.railway.app/\n\n' +
                            ...
                            if (routed) {
                                ...
                                if (routedCommand) {
                                    const senderIsAdminNexty = !isGroup || senderIsOwner
                                    ...
                                    const routedVerdict = access.authorizeCommand(routedCommand, {
                                        isGroup, isOwner: senderIsOwner, isSudo: this._isSudoUser(phoneNumber, sender),
                                        isMod: this._isModUser(phoneNumber, sender), isAdmin: senderIsAdminNexty,
                                    });
                                    if (!routedVerdict.allowed) {
                                        if (routedVerdict.reply) await reply(routedVerdict.reply);
                                        continue;
                                    }
                                    if (!['antilink'].includes(routed.commandName) || senderIsAdminNexty) {
                                        await routedCommand.execute({
                                            ...
                                            isOwner: senderIsOwner,
                                            isRealOwner: senderIsOwner,
                                            isSudo: this._isSudoUser(phoneNumber, sender),
                                            isAdmin: senderIsAdminNexty,
                            ...
                            const { getNextyAIReply } = require('../commands/ai/nexty');
                            ...
                            let nextyError = null;
                            const aiReply = nextyMedia
                                ? await require('../utils/smartAI').askMultimodal({
                                    key: 'nexty:' + phoneNumber + ':' + chatKey,
                                    system: 'You are Nexty, the sharp, concise AI personality of NEXTY MINI 👀. Analyze attached photos or sampled video frames accurately. State what is directly visible, distinguish uncertainty, and never invent details.',
                                    user: trigger.text,
                                    media: nextyMedia.media,
                                    remember: true,
                                    compact: true,
                                }).catch(error => {
                                    nextyError = error;
                                    console.error('[NextyAI] vision provider failed:', error.message);
                                    return null;
                                })
                                : await getNextyAIReply(
                                    concisePrompt(trigger.text, knowledge),
                                    'nexty:' + phoneNumber + ':' + chatKey,
                                    { memoryContext: nextyMemory?.getContext(database, chatKey) },
                                ).catch(error => {
                                nextyError = error;
                                console.error('[NextyAI] provider failed:', error.message);
                                return null;
                            });
                            ...
                            if (aiReply) {
                                ...
                                const failure = aiStatus.lastError || nextyError;
                                let detail = '';
                        ...
                        console.error('[NextyAI]', e.message);
                    }
                    continue;
                }
                ...
                // compatibility mode.
                sock.__nextyDeviceMode = database.getDeviceMode();
                ...
                // ── Central permission check (mode + ownerOnly/sudoOnly/adminOnly/groupOnly) ──
                // Private mode was already enforced by the gate above; this repeats
                // it defensively and applies the per-command metadata.
                let senderIsAdmin = senderIsOwner; // owner always counts as admin
                if (isGroup && !senderIsOwner && true) {
                    senderIsAdmin = await this._isGroupAdmin(sock, from, sender);
                }
                const verdict = access.authorizeCommand(command, {
                    isGroup, isOwner: senderIsOwner, isSudo: senderIsSudo,
                    isMod: senderIsMod, isAdmin: senderIsAdmin,
                });
                if (!verdict.allowed) {
                    if (verdict.reply) await reply(verdict.reply);
                    continue;
                }
                ...
                const context = {
                    ...
                    isOwner: senderIsOwner || (senderIsMod && command.category === 'owner' && !command.ownerOnly),
                    isRealOwner: senderIsOwner,
                    isSudo: senderIsSudo,
                    isMod: senderIsMod,
    ...
    async handleButtonResponse(sock, phoneNumber, msg, buttonId) {
        ...
        // Defensive: buttons must never bypass the private-mode gate even if a
        // future caller forgets the gate in handleMessages().
        const senderIsSudoBtn = this._isSudoUser(phoneNumber, sender);
        if (!access.canReceiveReplies({ isOwner: senderIsOwner, isSudo: senderIsSudoBtn })) return;
        ...
        if (String(buttonId).startsWith('nexty_logo:')) {
            try {
        ...
        if (String(buttonId).startsWith('chroma:')) {
            ...
            const senderIsMod = this._isModUser(phoneNumber, sender);
            let isAdmin = senderIsOwner;
            if (isGroup && !senderIsOwner) isAdmin = await this._isGroupAdmin(sock, from, sender).catch(() => false);
            const btnVerdict = access.authorizeCommand(command, {
                isGroup, isOwner: senderIsOwner, isSudo: senderIsSudoBtn, isMod: senderIsMod, isAdmin,
            });
            if (!btnVerdict.allowed) {
                if (btnVerdict.reply) await reply(btnVerdict.reply);
                return;
            }
            const isOwner = senderIsOwner || (senderIsMod && command.category === 'owner' && !command.ownerOnly);
    ...
    _isAllowed(num, phoneNumber, meta) {
        ...
        try {
            const sudo = database.getSudoUsers(phoneNumber) || [];
            if (sudo.map(j => this._normJid(j)).includes(num)) return true;
        } catch (_) {}
        if (meta?.owner) {
    ...
    async _handleAntiHijack(sock, phoneNumber, { id, participants, action, author }) {
        try {
            ...
            // The promotionGuard engine (configured by .antihijack) owns reversal; never run both.
            try { const pg = require('./promotionGuard').getGroupConfig(id); if (pg && (pg.antipromote || pg.antidemote)) return; } catch (_) {}
            ...
            const warn =
                ...
                `╰────────────⛧`;
            if (!access.isPrivate()) tasks.push(sock.sendMessage(id, {
                text: warn,
