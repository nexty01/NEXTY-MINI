            ...
            sock.ev.on('messages.upsert', async ({ messages, type }) => {
                ...
                for (const m of messages) {
                    try {
                        ...
                        if (grp.antiedit) {
                            const editProtocol = m.message?.protocolMessage;
                            const editedWrapper =
                                editProtocol?.type === 14
                                    ? editProtocol
                                    : m.message?.editedMessage || null;

                            const editedKey = editProtocol?.key ||
                                m.message?.editedMessage?.key ||
                                m.key?.originalKey || null;
                            if (editedKey && editedWrapper) {
                                ...
                                const newBody =
                                    ...
                                    editedWrapper.editedMessage?.imageMessage?.caption ||
                                    editedWrapper.editedMessage?.videoMessage?.caption ||
                                    editedWrapper.message?.conversation ||
                                    editedWrapper.message?.extendedTextMessage?.text ||
                                    editedWrapper.conversation ||
                                    editedWrapper.extendedTextMessage?.text ||
                                    '_(media/unknown)_';
                                ...
                                await sock.sendMessage(alertJid, {
                                    text:     notice,
                                    mentions: [sender]
                                });
                            }
                            ...
                            if (proto?.type === 0 && proto?.key) {
                                ...
                                for (const [msgKey, info] of Object.entries(mediaMap)) {
                                    ...
                                    try {
                                        ...
                                        await sock.sendMessage(alertJid, payload);
